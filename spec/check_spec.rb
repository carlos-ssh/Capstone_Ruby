...
describe CheckError do
  ...
  describe '#tag_error' do
    it "returns missing/unexpected tags eg '( )', '[ ]', and '{ }'" do
      checker.tag_error
      expect(checker.errors[0]).to eq("line:3 Lint/Syntax: Unexpected/Missing token ']' Square Bracket")
    end
  end
  ...
  describe '#empty_line_error' do
    it 'returns empty line error' do
      ...
      expect(checker.errors[0]).to eq('line:11 Extra empty line detected at block body end')
    end
  end
end
